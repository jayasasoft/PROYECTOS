...
$wp_reservations_edit = NULL; // Initialize page object first

class cwp_reservations_edit extends cwp_reservations {
	...
	// Page object name
	var $PageObjName = 'wp_reservations_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "") {
			$this->Page_Terminate("wp_reservationslist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// arrival
			$this->arrival->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->arrival->CurrentValue, 0), ew_CurrentDate(), $this->arrival->ReadOnly);

			// departure
			$this->departure->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->departure->CurrentValue, 0), ew_CurrentDate(), $this->departure->ReadOnly);

			// user
			$this->user->SetDbValueDef($rsnew, $this->user->CurrentValue, 0, $this->user->ReadOnly);
			...
			// approve
			$this->approve->SetDbValueDef($rsnew, $this->approve->CurrentValue, "", $this->approve->ReadOnly);

			// room
			$this->room->SetDbValueDef($rsnew, $this->room->CurrentValue, NULL, $this->room->ReadOnly);

			// roomnumber
			$this->roomnumber->SetDbValueDef($rsnew, $this->roomnumber->CurrentValue, "", $this->roomnumber->ReadOnly);

			// number
			$this->number->SetDbValueDef($rsnew, $this->number->CurrentValue, 0, $this->number->ReadOnly);

			// childs
			$this->childs->SetDbValueDef($rsnew, $this->childs->CurrentValue, 0, $this->childs->ReadOnly);

			// price
			$this->price->SetDbValueDef($rsnew, $this->price->CurrentValue, "", $this->price->ReadOnly);

			// custom
			$this->custom->SetDbValueDef($rsnew, $this->custom->CurrentValue, "", $this->custom->ReadOnly);

			// customp
			$this->customp->SetDbValueDef($rsnew, $this->customp->CurrentValue, "", $this->customp->ReadOnly);

			// reservated
			$this->reservated->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->reservated->CurrentValue, 0), ew_CurrentDate(), $this->reservated->ReadOnly);
...
// Create page object
if (!isset($wp_reservations_edit)) $wp_reservations_edit = new cwp_reservations_edit();

// Page init
$wp_reservations_edit->Page_Init();

// Page main
$wp_reservations_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$wp_reservations_edit->Page_Render();
?>
...
var CurrentForm = fwp_reservationsedit = new ew_Form("fwp_reservationsedit", "edit");

// Validate form
fwp_reservationsedit.Validate = function() {
...
fwp_reservationsedit.Form_CustomValidate = 
...
fwp_reservationsedit.ValidateRequired = true;
<?php } else { ?>
fwp_reservationsedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$wp_reservations_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $wp_reservations_edit->ShowPageHeader(); ?>
<?php
$wp_reservations_edit->ShowMessage();
?>
<form name="fwp_reservationsedit" id="fwp_reservationsedit" class="<?php echo $wp_reservations_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($wp_reservations_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $wp_reservations_edit->Token ?>">
<?php } ?>
...
<?php if ($wp_reservations->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_wp_reservations_id" class="col-sm-2 control-label ewLabel"><?php echo $wp_reservations->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $wp_reservations->id->CellAttributes() ?>>
<span id="el_wp_reservations_id">
<span<?php echo $wp_reservations->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $wp_reservations->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="wp_reservations" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($wp_reservations->id->CurrentValue) ?>">
<?php echo $wp_reservations->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $wp_reservations_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fwp_reservationsedit.Init();
</script>
<?php
$wp_reservations_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$wp_reservations_edit->Page_Terminate();
?>
