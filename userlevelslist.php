...
class cuserlevels_list extends cuserlevels {
	...
	function __construct() {
		...
		$this->AddUrl = "userlevelsadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
	...
	function Page_Init() {
		...
		if (@$_POST["ajax"] == "autofill") {
			...
			// Process auto fill for detail table 'userlevelpermissions'
			if (@$_POST["grid"] == "fuserlevelpermissionsgrid") {
				if (!isset($GLOBALS["userlevelpermissions_grid"])) $GLOBALS["userlevelpermissions_grid"] = new cuserlevelpermissions_grid;
				$GLOBALS["userlevelpermissions_grid"]->Page_Init();
				$this->Page_Terminate();
				exit();
			}
	...
	function SetupListOptions() {
		...
		$item = &$this->ListOptions->Add("detail_userlevelpermissions");
		$item->CssStyle = "white-space: nowrap;";
		...
		// Set up detail pages
		$pages = new cSubPages();
		$pages->Add("userlevelpermissions");
	...
	function RenderListOptions() {
		...
		// "detail_userlevelpermissions"
		$oListOpt = &$this->ListOptions->Items["detail_userlevelpermissions"];
		if ($Security->IsLoggedIn()) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("userlevelpermissions", "TblCaption");
			...
			$links = "";
			if ($GLOBALS["userlevelpermissions_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
			...
			if ($GLOBALS["userlevelpermissions_grid"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_userlevelpermissions");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=userlevelpermissions");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["userlevelpermissions"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["userlevelpermissions"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn());
		if ($item->Visible) {
...
<?php if ($userlevels->userlevelname->Visible) { // userlevelname ?>
	...
	<?php } else { ?>
		<th data-name="userlevelname"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $userlevels->SortUrl($userlevels->userlevelname) ?>',1);"><div id="elh_userlevels_userlevelname" class="userlevels_userlevelname">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $userlevels->userlevelname->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($userlevels->userlevelname->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($userlevels->userlevelname->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
